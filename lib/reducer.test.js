...
describe("Reducer", () => {
  ...
  it("should deep merge correctly", () => {
    const testCases = [
      {
        a: {
          nested: {
    ...
    testCases.forEach(testCase => {
      expect(deepMerge(testCase.a, testCase.b).toJS()).toEqual(
        testCase.expected,
      );
    });
  });
  ...
  describe("Busts query cache", () => {
    it("removes the query if it exists", () => {
      const request = { hash: 904239430, ql: `{ users { id } }` };
      const initialState = reducer(
        undefined,
        actions.queryStarted({
      ...
      const subject = reducer(
        initialState,
        actions.bustQueryCache("904239430"),
      );

      expect(subject).toMatchSnapshot();
    });

    it("doesn't crash if query doesn't exist", () => {
      const request = { hash: 904239430, ql: `{ users { id } }` };
      const initialState = reducer(
        undefined,
        actions.queryStarted({
      ...
      const subject = () =>
        reducer(initialState, actions.bustQueryCache("904239432"));
